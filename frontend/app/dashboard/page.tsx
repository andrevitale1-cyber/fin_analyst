...
const COLUMN_DEFINITIONS = [
  ...
  { key: 'soma_total', label: 'Soma', align: 'center', bg: 'bg-blue-100 dark:bg-blue-900/10', color: 'text-blue-800 dark:text-blue-200' },
  { key: 'qtde_tri', label: 'Resultados Analisados', align: 'center', bg: 'bg-purple-100 dark:bg-purple-900/10', color: 'text-purple-800 dark:text-purple-200' },
  { key: 'media', label: 'Média', align: 'center', bg: 'bg-green-100 dark:bg-green-900/10', color: 'text-green-800 dark:text-green-200' },
...
function Feature({ text, active = false, disabled = false }: any) {
  return (
    <li className="flex items-center gap-3">
      {disabled ? (
        <div className="p-0.5 rounded-full border border-gray-400 dark:border-gray-600 text-gray-400 dark:text-gray-500"><X size={12} /></div>
      ) : (
        <div className="p-0.5 rounded-full bg-green-100 dark:bg-green-500/10 text-green-600 dark:text-green-500 border border-green-300 dark:border-green-500/30">
          <Check size={12} />
        </div>
      )}
      <span className={`text-sm ${disabled ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-700 dark:text-gray-300'}`}>{text}</span>
    </li>
  );
}
      ...
      <div className="relative w-full max-w-3xl">
        ...
        <div className="flex flex-col items-center gap-1 mb-4">
          <div className="flex items-center justify-center gap-3">
            <span
              className={`text-base font-bold cursor-pointer transition-colors ${billingCycle === 'monthly' ? 'text-white' : 'text-gray-400'}`}
            ...
            <button 
              ...
              className="w-16 h-8 bg-gray-800 border border-gray-600 rounded-full p-1 relative transition-colors hover:bg-gray-700"
            >
            ...
            <span
              className={`text-base font-bold cursor-pointer transition-colors ${billingCycle === 'yearly' ? 'text-white' : 'text-gray-400'}`}
        ...
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="bg-white dark:bg-[#161b22] border border-gray-200 dark:border-gray-800 rounded-2xl p-5 flex flex-col h-full opacity-90 scale-95">
            ...
            <button onClick={onClose} className="block w-full text-center py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-white font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition-all mt-auto">
              Continuar Grátis
            </button>
          </div>
          ...
          <div className="bg-gray-50 dark:bg-[#0f131a] border-2 border-blue-500 rounded-2xl p-5 relative shadow-2xl shadow-blue-900/10 transform hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
            ...
            <h3 className="text-lg font-bold text-blue-600 dark:text-blue-400 mb-1">Premium</h3>
            <div className="flex items-end gap-1 mb-2">
              <span className="text-3xl font-bold text-gray-900 dark:text-white">{billingCycle === 'monthly' ? 'R$ 29' : 'R$ 290'}</span>
              <span className="text-gray-500 mb-1 text-lg">{billingCycle === 'monthly' ? '/mês' : '/ano'}</span>
            </div>
            <p className="text-gray-600 dark:text-gray-400 text-sm mb-3">Desbloqueie todo o poder da IA.</p>
...
function NavItem({ icon, label, active = false, onClick, isLocked = false, collapsed = false }: any) {
  return (
    <button onClick={onClick} title={collapsed ? label : undefined} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group relative ${collapsed ? 'justify-center px-2' : ''} ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20 dark:shadow-blue-900/20' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/5'}`}>
...
export default function FinancialDashboard() {
  ...
  const renderCellContent = (item: any, key: string, colDef: any) => {
    ...
    if (key === 'trimestre') return <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 py-1 px-2 rounded text-xs font-bold border border-blue-200 dark:border-blue-500/20">{item[key]}</span>;
